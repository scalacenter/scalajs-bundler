...
object NpmAssets {
  ...
  /** {{{
    ...
    *
    * @return
    *   A task producing a sequence of files paired with their path relative to the `node_modules` directory of
    *   `project`.
    * @param project
    *   The project that depends on the NPM package containing the assets to publish.
    * @param assets
    *   A function that finds files given the `node_modules` directory of `project`
    */
