...
class SBTBundlerFile(f: BundlerFile.Public) {
  ...
  def asAttributedFiles: Seq[sbt.Attributed[sbt.File]] = {
    ...
    val assets = Attributed
      .blankSeq(f.attributedFiles._2)
      .map {
        _.put(ProjectNameAttr, f.project)
...
object SBTBundlerFile {
  ...
  /** A [[scalajsbundler.BundlerFileType]], allowing unambigiuous selection of output files downstream from the plugin.
    */
  val BundlerFileTypeAttr: AttributeKey[BundlerFileType] = AttributeKey("bundler-file-type")
}
