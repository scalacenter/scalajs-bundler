...
/** This plugin enables `ScalaJSPlugin` and sets the `scalaJSModuleKind` to `CommonJSModule`. It also makes it possible
  * to define dependencies to NPM packages and provides tasks to fetch them or to bundle the application with its
  * dependencies.
  *
  * =Tasks and Settings=
  *
  * The [[ScalaJSBundlerPlugin.autoImport autoImport]] member documents the keys provided by this plugin. Besides these
  * keys, the following existing keys also control the plugin:
  *
  * ==`version in webpack`==
  *
  ...
  *
  * The directory in which NPM dependencies will be fetched, and where all the .js files will be generated. The
  * directory is different according to the current `Configuration` (either `Compile` or `Test`).
  *
  * Defaults to `crossTarget.value / "scalajs-bundler" / "main"` for `Compile` and `crossTarget.value /
  * "scalajs-bundler" / "test"` for `Test`.
  */
object ScalaJSBundlerPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    /** Installs NPM dependencies and all JavaScript resources found on the classpath as node packages.
      *
      * The JavaScript resources are installed locally in `node_modules` and can be used any other node package, such as
      * to load a module using `require()`.
      *
      ...
      *
      * The plugin uses different directories according to the configuration (`Compile` or `Test`). Thus, this setting
      * is scoped by a `Configuration`.
      *
      * The task returns the directory in which the dependencies have been fetched (the directory that contains the
    ...
    /** Installs NPM dependencies.
      *
      * Unlike [[npmUpdate]] this does not stage the javascript resources and so is safe to use in `sourceGenerators` or
      * any other task that is used by `fullClasspath`.
      *
      * The plugin uses different directories according to the configuration (`Compile` or `Test`). Thus, this setting
      * is scoped by a `Configuration`.
      *
      * Typically, if you want to define a task that uses the downloaded NPM packages you should specify the
    ...
    /** List of the NPM packages (name and version) your application depends on. You can use
      * [semver](https://docs.npmjs.com/misc/semver) versions:
      *
      * {{{
    ...
    /** Map of NPM packages (name -> version) to use in case transitive NPM dependencies refer to a same package but
      * with different version numbers. In such a case, this setting defines which version should be used for the
      * conflicting package. Example:
      *
      * {{{
      *   npmResolutions in Compile := Map("react" -> "15.4.1")
      * }}}
      *
      * If several Scala.js projects depend on different versions of `react`, the version `15.4.1` will be picked. But
      * if all the projects depend on the same version of `react`, the version given in `npmResolutions` will be
      * ignored.
      *
      * If different versions of the packages are referred but the package is NOT configured in `npmResolutions`, a
      * version conflict resolution is delegated to npm/yarn. This behavior may reduce a need to configure
    ...
    /** List of the additional configuration options to include in the generated 'package.json'. Note that package
      * dependencies are automatically generated from `npmDependencies` and `npmDevDependencies` and should '''not''' be
    ...
    /** [[scalajsbundler.BundlingMode]] to use.
      *
      * Must be one of: `Application` - Process the entire Scala.js output file with webpack, producing a bundle
      * including all dependencies `LibraryOnly()` - Process only the entrypoints via webpack and produce a library of
      * dependencies `LibraryAndApplication() - Process only the entrypoints, concatenating the library with the
      * application to produce a bundle
      *
      * The default value is `Application`
      */
    val webpackBundlingMode: SettingKey[BundlingMode] =
      settingKey[BundlingMode](
        "Bundling mode, one of BundlingMode.{ Application,  LibraryOnly, LibraryAndApplication }.")

    /** Bundles the output of a Scala.js stage.
      *
      * This task must be scoped by a Scala.js stage (either `fastOptJS` or `fullOptJS`) and a `Configuration` (either
      * `Compile` or `Test`).
      *
      ...
      * }}}
      *
      * Note that to scope the task to a different project than the “current” sbt project, you have to write the
      * following:
      *
      * {{{
      ...
      *
      * The task is cached, so webpack is only launched when some of the used files have changed. The list of files to
      * be monitored is provided by webpackMonitoredFiles
      *
      ...
      * [[scalajsbundler.sbtplugin.SBTBundlerFile.ProjectNameAttr]] contains the "prefix" of the file names, such as
      * `yourapp-fastopt`, while the [[scalajsbundler.sbtplugin.SBTBundlerFile.BundlerFileTypeAttr]] contains the bundle
      * file type, which can be used to filter the list of files by their [[scalajsbundler.BundlerFileType]]. For
      * example:
      *
      * {{{
    ...
    /** configuration file to use with webpack. By default, the plugin generates a configuration file, but you can
      * supply your own file via this setting. Example of use:
      *
      * {{{
    ...
    /** whether to enable (or not) source-map in a given configuration (`Compile` or `Test`) and stage (`fastOptJS` or
      * `fullOptJS`). Example of use:
      *
      * {{{
      *   webpackEmitSourceMaps in (Compile, fullOptJS) := false
      * }}}
      *
      * By default, this setting is undefined and scalajs-bundler fallbacks to Scala.js’ `sourceMap` setting, so, to
      * globally disable source maps you can just configure the `sourceMap` setting:
      *
      * {{{
    ...
    /** Additional directories, monitored for webpack launch.
      *
      * Changes to files in these directories that match `includeFilter` scoped to `webpackMonitoredFiles` enable
    ...
    /** Whether to use [[https://yarnpkg.com/ Yarn]] to fetch dependencies instead of `npm`. Yarn has a caching
      * mechanism that makes the process faster.
      *
  ...
  private[sbtplugin] val scalaJSBundlerWebpackConfig =
    TaskKey[BundlerFile.WebpackConfig](
  ...
  override lazy val projectSettings: Seq[Def.Setting[_]] = Seq(
    ...
    ensureModuleKindIsCommonJSModule := {
      if (scalaJSLinkerConfig.value.moduleKind == ModuleKind.CommonJSModule) true
        ...
        sys.error(
          s"scalaJSModuleKind must be set to ModuleKind.CommonJSModule in projects where ScalaJSBundler plugin is enabled")
    },
    ...
    installJsdom := {
      ...
      if (!jsdomDir.exists()) {
        ...
        addPackages(baseDir, installDir, useYarn.value, log, npmExtraArgs.value, yarnExtraArgs.value)(
  ...
  private def perScalaJSStageSettings(stage: Stage): Seq[Def.Setting[_]] = {
    ...
    Seq(
      ...
      webpackMonitoredFiles in stageTask := {
        ...
        val generatedFiles: Seq[File] = Seq(packageJsonFile.file, generatedWebpackConfigFile.file, entry.file)
      ...
      startWebpackDevServer in stageTask := Def
        .task {
