...
private[scalajsbundler] class WebpackDevServer {
  ...
  /** @param workDir
    *   path to working directory for webpack-dev-server
    * @param configPath
    *   path to webpack config.
    * @param extraArgs
    *   additional arguments for webpack-dev-server.
    * @param logger
    *   a logger to use for output
    * @param globalLogger
    *   a global logger to use for output even when the task is terminated
    */
  def start(
      workDir: File,
      configPath: File,
