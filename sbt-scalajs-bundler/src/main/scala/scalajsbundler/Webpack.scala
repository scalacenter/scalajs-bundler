...
object Webpack {
  ...
  def bundle(
    ...
    val stats = Webpack.run(nodeFlags: _*)(args: _*)(targetDir, log)
  ...
  def bundleLibraries(
    ...
    val args = extraArgs ++: Seq("--config", configFile.absolutePath)
    val stats = Webpack.run(nodeFlags: _*)(args: _*)(generatedWebpackConfigFile.targetDir.toFile, log)
    stats.foreach(_.print(log))
  ...
  /**
    ...
    *
    * @param nodeFlags node.js cli flags
    ...
    * @param log Logger
    */
  def run(nodeFlags: String*)(args: String*)(workingDir: File, log: Logger): Option[WebpackStats] = {
    val webpackBin = workingDir / "node_modules" / "webpack" / "bin" / "webpack"
    val cmd = Seq("node") ++ nodeFlags ++ Seq(webpackBin.absolutePath, "--bail", "--profile", "--json") ++ args
